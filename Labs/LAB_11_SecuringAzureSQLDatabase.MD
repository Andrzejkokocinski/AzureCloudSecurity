...
6. On the **Custom deployment** blade, ensure that the following settings are configured (leave any others with their default values):
   ...
   |Resource group|click **Create new** and type the name **lab11LAB11**|
   |Location|**(US) East US**|
...
2. On the **Resource groups** blade, in the list of resource group, click the **lab11LAB11** entry.

3. On the **lab11LAB11** blade, click the entry representing the newly created SQL Server.
...
2. In the list of databases, select the **lab11LabDb** entry. 

3. On the **lab11LabDb** SQL database blade, in the **Security** section, click **Data Discovery & Classification**.
...
11. In the list of databases, select the **lab11LabDb** entry. 

12. On the **lab11LabDb** SQL database blade, in the **Security** section, click **Auditing**. 
...
3. In the PowerShell session within the Cloud Shell pane, run the following to remove the resource group you created in this lab:
    ...
    ```powershell
    Remove-AzResourceGroup -Name "lab11LAB11" -Force -AsJob
    ```
