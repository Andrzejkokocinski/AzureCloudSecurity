...
6. On the **Custom deployment** blade, under **Deployment Scope** ensure that the following settings are configured (leave any others with their default values):
   ...
   |Resource group|click **Create new** and type the name **lab10LAB10**|
   |Location|**East US**|
...
3. In the PowerShell session within the Cloud Shell pane, run the following to create an Azure Key Vault in the resource group **lab10LAB10**. (If you chose another name for this lab's Resource Group out of Task 1, use that name for this task as well). The Key Vault name must be unique. Remember the name you have chosen. You will need it throughout this lab.  

    ```powershell
    $kvName = 'lab10kv' + $(Get-Random)

    $location = (Get-AzResourceGroup -ResourceGroupName 'lab10LAB10').Location

    New-AzKeyVault -VaultName $kvName -ResourceGroupName 'lab10LAB10' -Location $location
    ```
...
6. On the **Resource groups** blade, in the list of resource group, click the **lab10LAB10** (or other name you chose earlier for the resource group) entry.
...
3. In the PowerShell session within the Cloud Shell pane, run the following to add a software-protected key to the Key Vault: 
    ...
    ```powershell
    $kv = Get-AzKeyVault -ResourceGroupName 'lab10LAB10'
...
4. In the PowerShell session within the Cloud Shell pane, run the following to create a variable storing the Key Vault name.
	```
    $kvName = (Get-AzKeyVault -ResourceGroupName 'lab10LAB10').VaultName
...
5. In the PowerShell session within the Cloud Shell pane, run the following to grant permissions on the Key Vault to the application you registered in the previous task:
    ...
    ```powershell
    Set-AZKeyVaultAccessPolicy -VaultName $kvName -ResourceGroupName lab10LAB10 -ServicePrincipalName $applicationId -PermissionsToKeys get,wrapKey,unwrapKey,sign,verify,list
    ```
...
2. In the list of SQL databases, click the **medical(<randomsqlservername>)** entry.
    ...
    >**Note**: If the database cannot be found, this likely means the deployment you initiated in Exercise 1 has not completed yet. You can validate this by browsing to the Azure Resource Group "lab10LAB10" (or the name you chose), and selecting **Deployments** from the Settings pane.  
...
In this task, you log on to the Azure VM, which deployment you initiated in Exercise 1. This Azure VM hosts Visual Studio 2019 and SQL Server Management Studio 2018.
    ...
    >**Note**: Before you proceed with this task, ensure that the deployment you initiated in the first exercise has completed successfully. You can validate this by navigating to the blade of the Azure resource group "lab10Lab10" (or other name you chose) and selecting **Deployments** from the Settings pane.  

1. In the Azure portal, in the **Search resources, services, and docs** text box at the top of the Azure portal page, type **virtual machines** and press the **Enter** key.

2. In the list of Virtual Machines shown, select the **lab10-10-vm1** entry. On the **lab10-10-vm1** blade, on the **Essentials** pane, take note of the **Public IP address**. You will use this later. 
...
2. On the **Firewall settings** blade, scroll down to Rule Name, click **+ Add a firewall rule**, and specify the following settings: 
    ...
    |Start IP|the Public IP Address of the lab10-10-vm1|
    |End IP|the Public IP Address of the lab10-10-vm1|
...
4. Navigate back to the **lab10-10-vm1** blade, click **Overview**, next click **Connect** and, in the drop down menu, click **RDP**. 

5. Click **Download RDP File** and use it to connect to the **lab10-10-vm1** Azure VM via Remote Desktop. When prompted to authenticate, provide the following credntials:
    ...
    >**Note**: The remaining steps in this lab are performed within the Remote Desktop session to the **lab10-10-vm1** Azure VM. 
...
1. From the RDP session to the **lab10-10-vm1**, launch **Visual Studio 2019** from the **Start menu**.
...
3. In the PowerShell session within the Cloud Shell pane, run the following to remove the resource groups you created in this lab:
    ...
    ```powershell
    Remove-AzResourceGroup -Name "lab10LAB10" -Force -AsJob
    ```
