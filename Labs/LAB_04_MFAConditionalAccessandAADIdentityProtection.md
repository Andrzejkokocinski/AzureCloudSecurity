...
>**Note**: You will need to create a unique password that will be used for creating VMs (virtual machines) for the rest of the course. The password must be at least 12 characters long and meet the defined complexity requirements (Password must have 3 of the following: 1 lower case character, 1 upper case character, 1 number, and 1 special character). [VM password requirements](https://docs.microsoft.com/en-us/azure/virtual-machines/windows/faq#what-are-the-password-requirements-when-creating-a-vm-). Please make a note of the password.
   ...
   |Resource group|click **Create new** and type the name **lab04LAB04**|
   |Location|**East US**|
   |Vm Size|**Standard_D2s_v3**|
   |Vm Name|**lab04-04-vm1**|
   ...
   |Virtual Network Name|**lab04-04-vnet1**|
...
> Result: You have initiated a template deployment of an Azure VM **lab04-04-vm1** that you will use in the last exercise of this lab.
...
5. On the **New** blade, configure the following settings:
   ...
   - In the **Name** text box, type **lab04Policy1**
...
15. On the **Conditional Access \| Policies** blade, click the ellipsis next to **lab04Policy1**, click **Delete**, and, when prompted to confirm, click **Yes**.
...
4. Type the following policy name in the **Name** text box, type **lab04Policy2**.
...
20. Click **Other**, and type the following in the field: **lab04 lab use**.
...
3. Type the following policy name in the **Name** text box, type **lab04Policy3**.
...
> Before you start this task, ensure that the template deployment you started in Exercise 1 has completed. The deployment includes an Azure VM named **lab04-04-vm1**. 

1. In the Azure portal, set the **Directory + subscription** filter to the the Azure AD tenant associated with the Azure subscription into which you deployed the **lab04-04-vm1** Azure VM.
...
3. On the **Virtual machines** blade, click the **lab04-04-vm1** entry. 

4. On the **lab04-04-vm1** blade, click **Connect** and, in the drop down menu, click **RDP**. 

5. Click **Download RDP File** and use it to connect to the **lab04-04-vm1** Azure VM via Remote Desktop. When prompted to authenticate, provide the following credntials:
   ...
   |Password|**Please use your personal password created in Lab 04 > Exercise 1 > Task 1 > Step 9.**|
    ...
    >**Note**: The following steps are performed in the Remote Desktop session to the **lab04-04-vm1** Azure VM. 
...
4. On the **lab04-04-vm1** blade, click **Stop** and, when prompted to confirm, click **OK** 
